...
-- Strategy: aggregate per-day revenue, then compute a running total.
-- Incremental runs reprocess a rolling window to capture late-arriving events
-- using Spark's insert_overwrite partition strategy, which efficiently overwrites
-- only the affected partitions while preserving older, stable partitions.
